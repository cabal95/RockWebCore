...
namespace RockWebCore.Routing
{
    ...
    public class RockRouterMiddleware
    {
        ...
        private async Task PageRouteAsync( HttpContext context )
        {
            ...
            rockRequestContext.CurrentPage = new RockPage( pageCache, rockRequestContext );
        ...
        /// <summary>
        /// Executes the request for a shortlink.
        /// </summary>
        /// <param name="context">The context.</param>
        /// <returns></returns>
        private Task ShortLinkAsync( HttpContext context )
        {
            var routeData = context.GetRouteData();
            int shortlinkId = ( int ) routeData.Values["ShortLinkId"];
            string url = ( string ) routeData.Values["Url"];
            string token = ( string ) routeData.Values["Token"];
            var rockRequestContext = context.RequestServices.GetRequiredService<RockRequestContext>();

            var transaction = new Rock.Transactions.ShortLinkTransaction
            {
                PageShortLinkId = shortlinkId,
                Token = token,
                Url = url,
                UserName = rockRequestContext.CurrentUser?.UserName,
